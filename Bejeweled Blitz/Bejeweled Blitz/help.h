...
bool isCursorKeyPressed(int& whichKey)   //whichKey passed as reference.... 
{
	...
	if (key == 1)
	{
		whichKey = 10;		// 10 if Enter(return) key is pressed 
		return true;
	}
	return false;
}
...
//--------------- For Making color filled rectangle---------------//
void myFilledRect(int x1, int y1, int x2, int y2, int R, int G, int B, int backR, int backG, int backB)   //back means background R,G,B code.
{
	HWND console_handle = GetConsoleWindow();
    HDC device_context = GetDC(console_handle);
    ...
    HPEN pen =CreatePen(PS_SOLID,2,RGB(R,G,B)); 
    SelectObject(device_context,pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(backR,backG,backB)); //Fill color can also be passed as parameter to the function!!!
